...
export async function GET(
  ...
  try {
    ...
    const costs = await connection.query(
      `SELECT * FROM general_costs WHERE id = ?`,
      [id]
    );
    ...
    return NextResponse.json({ error: 'Failed to fetch general cost' }, { status: 500 });
  } finally {
...
export async function PUT(
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to update general cost' }, { status: 500 });
  } finally {
    if (connection) connection.release();
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    await connection.query(`DELETE FROM general_costs WHERE id = ?`, [id]);
    ...
    return NextResponse.json({ error: 'Failed to delete general cost' }, { status: 500 });
  } finally {
    if (connection) connection.release();
  }
}
