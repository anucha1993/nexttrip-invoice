...
export async function GET() {
  ...
  try {
    ...
    // Add wholesaleId column if not exists
    try {
      await connection.query(`
        ALTER TABLE purchase_taxes 
        ADD COLUMN wholesaleId INT DEFAULT NULL COMMENT 'ID ของ Wholesale' AFTER quotationId
      `);
    } catch (e: any) {
      // Ignore if column already exists
      if (!e.message?.includes('Duplicate column')) {
        console.log('wholesaleId column may already exist');
      }
    }

    return NextResponse.json({ 
      success: true, 
      message: 'Purchase taxes table created/updated successfully' 
    });
  } catch (error) {
